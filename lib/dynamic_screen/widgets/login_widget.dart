...
class _LoginWidgetState extends State<LoginWidget> {
  ...
  _LoginWidgetState() {
    debugPrint('🔑 LoginWidget CREATED (ID: $_instanceId)');
  }

  @override
  void dispose() {
    debugPrint('🔑 LoginWidget DISPOSED (ID: $_instanceId)');
  ...
  Future<void> signIn() async {
    debugPrint('🔑 LoginWidget #$_instanceId - signIn() START');

    if (!(_formKey.currentState?.validate() ?? false)) {
      debugPrint('🔑 LoginWidget #$_instanceId - Form validation failed');
      return;
    }
    ...
    try {
      debugPrint('🔑 LoginWidget #$_instanceId - Calling FirebaseAuth.signInWithEmailAndPassword');
      ...
      debugPrint('✅ LoginWidget #$_instanceId - LOGIN SUCCESS: User ${cred.user?.uid}');
      debugPrint('🔑 LoginWidget #$_instanceId - NOT navigating manually. Waiting for DashboardScreen listener.');
      
      // ✅ FIX CONFIRMED: NO NAVIGATION CODE HERE!
      // DO NOT call Navigator.pushReplacement()
      // The DashboardScreen's authStateChanges listener will handle the transition.

    } on FirebaseAuthException catch (e) {
      debugPrint('❌ LoginWidget #$_instanceId - Auth Error: ${e.code} - ${e.message}');
      setState(() {
        _errorMessage = _getErrorMessage(e.code);
      });
    } catch (e) {
      debugPrint('❌ LoginWidget #$_instanceId - Unknown Error: $e');
      setState(() {
        _errorMessage = "Unexpected error. Please try again.";
      });
    } finally {
      if (mounted) {
        debugPrint('🔑 LoginWidget #$_instanceId - signIn() FINISHED, resetting loading state');
        setState(() {
          _isLoading = false;
        });
      } else {
         debugPrint('⚠️ LoginWidget #$_instanceId - Widget unmounted after signIn, skipping setState');
      }
    }
  }
