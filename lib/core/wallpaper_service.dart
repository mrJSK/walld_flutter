...
class WallpaperService extends ChangeNotifier {
  ...
  void setGlassBlur(double blur) {
    ...
    if (clampedBlur == 0.0 && oldBlur > 5.0) {
      debugPrint('🛡️ GLASS PROTECTED: Blocked blur=0.0, keeping $oldBlur (throttledNotify)');
      return; // BLOCK IT!
    }
  ...
  void setGlassOpacity(double opacity) {
    ...
    if (clampedOpacity < 0.05 && oldOpacity > 0.10) {
      debugPrint('🛡️ GLASS PROTECTED: Blocked opacity=$clampedOpacity, keeping $oldOpacity (throttledNotify)');
      return; // BLOCK IT!
    }
